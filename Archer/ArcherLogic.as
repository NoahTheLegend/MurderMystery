...
const int STAB_DELAY = 11;
const int STAB_TIME = 28;
...
void ManageGrapple(CBlob@ this, ArcherInfo@ archer)
{
		...
		&& this.get_u8("role") == 2)
	{
		...
		CBlob@[] blobs;
		getMap().getBlobsInRadius(aimpos, 8.0f, blobs);
		...
		for (int i = 0; i < blobs.size(); i++)
		{
			...
			if (target !is null && target.getPlayer() !is null
				&& target.get_u8("role") < 2
				&& Vec2f(target.getPosition() - pos).Length() <= stab_length)
			{
...
void ManageBow(CBlob@ this, ArcherInfo@ archer, RunnerMoveVars@ moveVars)
{
	...
	else if (this.isKeyPressed(key_action1) && hasArrows(this))
	{
		moveVars.walkFactor *= 0.75f;
...
void onTick(CBlob@ this)
{
	if (this.isMyPlayer())
	{
		getRules().set_u8("my_role", this.get_u8("role"));
	}
...
{
	if (!this.hasTag("defender")) return null;
	CBlob@[] blobsInRadius;
...
void onCollision(CBlob@ this, CBlob@ blob, bool solid, Vec2f normal, Vec2f point1)
{
	if (blob !is null && blob.hasTag("dead")
		&& blob.get_u8("role") == 1 && this.get_u8("role") == 0
			&& !blob.hasTag("role_given"))
	{
		if (isServer())
		{
			blob.Tag("role_given");
			this.Sync("role_given", true);
		...
		if (isClient())
		{
			this.getSprite().PlaySound("FastBowPull.ogg", 1.5f);
		}
	}
}
