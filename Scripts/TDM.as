...
void Config(TDMCore@ this)
{
	...
	s32 warmUpTimeSeconds = cfg.read_s32("warmUpTimeSeconds", 3);
	...
	f32 gameDurationMinutes = 1.0f + getPlayersCount() * 0.25f;
...
shared class TDMCore : RulesCore
{
	...
	void Update()
	{
		...
		if ((rules.isIntermission() || rules.isWarmup()) && count < min_ply)  //CHECK IF TEAMS HAVE ENOUGH PLAYERS
		{
			gametime = getGameTime() + warmUpTime;
			rules.set_u32("game_end_time", gametime + gameDuration);
			rules.SetGlobalMessage("Not enough players to start a game");
			tdm_spawns.force = true;
		}
	...
	{
		TDMPlayerInfo p(player.getUsername(), player.getTeamNum(), "archer");
		players.push_back(p);
	...
	void CheckTeamWon()
	{
		...
		if (murderers_left && !innocents_left)
		{
			conclusion = "All innocents are dead!";
			match_end = true;	
		}
		if (innocents_left && !murderers_left)
		{
			conclusion = "All murderers are dead!";
			match_end = true;
		}
		...
		if (match_end)
		{
			...
			rules.SetGlobalMessage(conclusion);
		}
	}
...
void Reset(CRules@ this)
{
	...
	this.set_s32("restart_rules_after_game_time", 8*30);
}

void onRestart(CRules@ this)
{
	Reset(this);
	this.Untag("waiting for restart");
}
